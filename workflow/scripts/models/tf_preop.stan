data{
    ...
    real cfDNA_mean; // mean k-mer count in cfDNA sample, gc-content stratified
...
parameters{
    ...
    real<lower=pow((cfDNA_mean*TF), 2)/((cfDNA_mean*TF*t_phi_lb_scale) - (cfDNA_mean*TF))> t_phi;
}     
...
model{
    ...
    t_phi ~ gamma(t_phi_a, t_phi_b) T[pow((cfDNA_mean*TF), 2)/((cfDNA_mean*TF*t_phi_lb_scale) - (cfDNA_mean*TF)), ]; 
